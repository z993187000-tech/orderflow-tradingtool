...
实时模式使用 Binance USDⓈ-M Futures WebSocket：

- Aggregate Trade Stream：`<symbol>@aggTrade`，用于成交明细、Delta、成交量分布。
- Book Ticker Stream：`<symbol>@bookTicker`，用于顶部 Last / 最新价的 bid/ask 中间价。
- Combined Endpoint：`wss://fstream.binance.com/stream?streams=<symbol>@aggTrade/<symbol>@bookTicker`
...
- Last / 最新价：优先显示 Binance `bookTicker` 的 bid/ask 中间价；如果盘口还没收到，临时回退为最新成交价格。
...
- 顶部 Last / 最新价优先使用 Binance `bookTicker` 的 bid/ask 中间价；订单流曲线和 Delta 仍使用 `aggTrade` 成交流。
